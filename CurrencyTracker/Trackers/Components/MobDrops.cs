...
public unsafe class MobDrops : TrackerComponentBase
{
    ...
    {
        if (flag != ConditionFlag.InCombat || DService.Instance().Condition.IsBoundByDuty) return;
    ...
    private bool? EndMobDropsHandler()
    {
        ...
        (
            items,
            "",
            $"({Service.Lang.GetText("MobDrops-MobDropsNote", string.Join(", ", EnemiesList.Values.TakeLast(3)))})",
