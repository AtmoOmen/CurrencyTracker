...
public class Transaction : IEquatable<Transaction>
{
    ...
    public static string SanitizeFilePath(string filePath)
    {
        ...
        foreach (var c in filePath) sanitizedPath.Append(Array.IndexOf(invalidChars, c) < 0 ? c : '_');
    ...
    public static Transaction FromFileLine(ReadOnlySpan<char> span)
    {
        ...
        if (parts.Length != 5) throw new FormatException("Invalid transaction format");
    ...
    public static async Task WriteTransactionsToFileAsync(string filePath, List<Transaction> transactions)
    {
        ...
        try
        {
            ...
            await using var writer     = new StreamWriter(fileStream);
            foreach (var transaction in transactions) await writer.WriteLineAsync(transaction.ToFileLine());
        }
        catch (IOException ex)
        {
            await TransactionsHandler.BackupTransactionsAsync(P.PlayerDataFolder, PluginConfig.Instance().MaxBackupFilesCount);
